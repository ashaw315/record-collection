...
## POST-A41 MEASUREMENT: output is where the pressure went, and 2.5x was wrong twice

**2026-08-26, third run to record tokens.** A41 verified in real use — every
reason now grounded in a real owned title, Bitches Brew replaced by *In A Silent
Way*, Gaucho by *Aja*. **The duplicate stopped**, which is what only a real run
could show.

| run | | input | output | headroom |
|---|---|---|---|---|
| 18:42 | pre-A41 | 1,603 | 526 | 7.6x |
| 19:10 | pre-A41 | 1,738 | 1,210 | 3.3x |
| **19:50** | **post-A41** | **1,886** | **1,736** | **2.3x** |

**Input: +148 tokens for 17 titles.** My estimate was ~150, so that half held —
and it extrapolates to ~1,700 at 200 records rather than the ~2,000 I quoted.
**Input is settled as a non-issue.**

**Output is where the pressure went, and A41 plausibly caused it.** A reason
grounded in a specific title is longer than an ungrounded one — *"Bitches Brew is
on your shelf, and this is the quieter session that opened the electric fusion
door it walked through"* costs more than *"a foundational record of a scene they
collect deeply"*. **That is the feature working and it costs tokens.**

**My 2.5x figure has now been wrong in two different ways.** First it was
extrapolated from a single sample, which cannot bound variance (A41). Now the
observed worst case has passed UNDER it: 2.3x. A figure published as a floor
turned out to be neither a floor nor a ceiling, just one sample's arithmetic
dressed as a bound.

**The number to carry is 2.3x, and the trend across three runs is one
direction.** Six is comfortable at every observed length; a further doubling
would not be. **What to watch is output per suggestion rather than the count** —
and `llm_requests` records it every run now, so this is queryable rather than
anecdotal. That is A38 paying for itself a third time.

---

## DECIDED: an acted-on suggestion STAYS in the displayed list

**Adam's question, 2026-08-26, after adding *Aja* to the want list from a
suggestion. His reading, and it is right.**

**His three arguments, all sound:** the stored result is a record of what Claude
said at 19:15, not a live list; mutating it would make "Asked 4 minutes ago" stop
describing what is on screen; and the item is excluded from the NEXT run anyway,
because the want list is on the wire.

**The stronger argument, which he did not quite make: removing it would destroy
the evidence the feature worked.** The list is what makes "I asked, and I acted
on one" legible. Strip acted-on items and a six-suggestion answer the user fully
used collapses to an empty panel that reads as a failed call — **absent versus
unknown, again.** A suggestion acted on is not stale; it is the record of a
completed action, which is the most useful state it reaches.

**And a consistency argument.** The panel already says "Generated by Claude" and
A39 shows when it was asked — both framing it as a transcript. A list that
quietly edited itself would contradict its own label, the way a "Suggest" button
returning a cached answer would.

### Where his self-doubt was right to exist, and why it does not apply

He asked whether "it is a record, not a queue" is *"a distinction that matters
more to the schema than to me looking at the screen"* — a real failure mode, and
this project has shipped correctness nobody benefits from.

**It does not apply here because the schema distinction and the screen
experience point the same way.** On screen an acted-on suggestion is INFORMATION
("I dealt with that one"), not clutter. The cost he worried about bites only for
a list that is long, unordered, and older than memory — a six-item list read four
minutes ago is none of those.

### NOT BUILT: marking what was acted on

**The option worth naming instead of removal.** A quiet marker on suggestions now
on the want list preserves the transcript, removes the re-reading cost, and ADDS
information rather than deleting it.

**Its cost is why it is not built on a hypothesis:** joining suggestions to the
want list means matching on `(artist, title)` STRINGS, which this project
distrusts for good reason — the same brittleness A40 flagged for identifying a
pressing, and the same reason §4.1 keys find-or-create on external ids rather
than names. A stored suggestion has no id to join on because it is a transcript
of what a model said, not a row in the collection.

**Trigger: Adam re-reading the list to remember what he acted on.** That is the
