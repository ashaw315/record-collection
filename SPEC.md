...
- **`pressings` rows are shared, not owned.** Before creating one, find-or-create by `discogs_release_id` if present, otherwise by the tuple `(catalog_number, country_pressed, year_pressed)`. A pressing may be referenced simultaneously by a `records` row and a `want_list.target_pressing_id`.
