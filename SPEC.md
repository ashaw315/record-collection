...
- ~~**A spine's colour is the average colour of its cover**~~ **A spine's colour is the DOMINANT CHROMATIC REGION of its cover (A75, 2026-09-12)**, computed once when the cover is attached and stored in `records.spine_colour` (§4.2). **The mechanism changed and the prohibition did not.** Averaging is the operation that destroys chroma: a sleeve with red, green and blue regions averages to neutral, and measured across all sixteen stored covers the mean returned twelve near-greys — for half of which the artwork holds a genuine accent the mean cancelled (Gaucho is 27.7% vivid, peaking at chroma 0.161, averaged to a sage grey). So "twelve of seventeen are near-grey" was a fact about the derivation, not about the covers.

  **This is NOT the most populous colour bucket, which stays rejected.** That rule gave a warm brown portrait a near-black spine by sampling the leather jacket — a wrong answer rather than a different one — and it remains rejected. What now prevents it is explicit: pixels are binned by **hue and lightness in OKLCH** and a bin qualifies only if it clears **both a 5% share floor and a 0.04 chroma floor**, so the winner is the most chromatic significant region rather than the largest one. A near-black region cannot win, because it fails the chroma floor. Parameters were chosen from a sweep across 64/96/128px reductions, 12×3/24×4/36×5 bins and 3%/5%/8% floors: reduction size barely matters, while fine bins at a high floor collapse the result by fragmenting one region below the floor, so the settled values sit in the stable middle of that sweep rather than on an edge.

  **A cover with no chromatic region keeps the linear-light, alpha-weighted mean**, and that fallback is explicit rather than emergent (`NO_CHROMATIC_REGION`) — six of the sixteen real covers reach it, because their artwork genuinely has no accent to find (The Money Store's most saturated single pixel is chroma 0.027). One record, `Loss Of Life`, has a dominant region *less* chromatic than its mean, and the chroma floor is what makes it take the fallback instead of getting a darker, duller spine than before — that case is why the floor exists. Saturation is never boosted: a spine is a claim about a cover, and a shelf prettier than the sleeves on it is inventing colour the record does not have. ~~A record with no cover gets a plain spine — an honest absence, not a gap in the wall.~~ **RETIRED (A62, 2026-09-09)** — a plain spine is not distinguishable from three records that have covers; see the blockquote at the head of this section.
- **Spine text is artist, title and catalogue number**, set in mono, rotated. The catalogue number is the collector's identifier and earns its space.
  ...
  **The derived budget is 36, not 44 (A62a, 2026-09-09; CORRECTED 2026-09-11).** SPEC.md never declares a character count; §11's unit-test bullet says the budget *derives from spine height*, and the code computes `floor(240 / 5.4)` = 44. **Geist Mono advances 0.6em per character, so the divisor is a function of the type size** — and at the 10.39px the isometric shear requires to read as 9px, the advance is **6.23px**, giving `floor(240 / 6.23)` = **36**. Two of seventeen labels exceed it. The code's 5.4 is the stale divisor and the budget it yields is 8 characters too generous.

  > ~~At the 9px legibility floor a 150px spine holds 25 characters set upright and 22 on an isometric face… a spine label cannot carry identity at any size the shelf offers.~~ **RETRACTED.** That conclusion was measured against a **150px spine, which the wall does not have** — one of several drawing scales, none of them the wall's. **`SPINE_HEIGHT` is 240**, from which `MIN_SPINE_WIDTH` = round(240/14) = 17 and `MAX` = round(240/10) = 24 derive. A 9px label occupies a **13.5-unit band across the spine** — an absolute floor does not shrink when the spine does — and 13.5 into 17 **fits, with 3.5 units spare on the thinnest record in the collection**. The label carries, at every record count, and overlap is impossible by construction.
  >
  > **The band measurement was the contribution and it survives the retraction**: nobody had measured the label's extent *across* the thickness, and the fit now rests on that rather than on an assumption. What failed was reasoning from a drawing convenience adopted as a constant — the same shape as A74's untested mechanism, one layer down: a figure asserted at a scale where its own constraint could not bind.

  **The sentence above defends the component that truncation removes first:** the catalogue number "earns its space" and is the last thing standing in the degenerate case, while the title — which this sentence also promises — is the first to go once the budget binds.
