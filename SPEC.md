...
**The stacked summary is not a smaller flanking panel — it is a different shape.** The panel beside the record carries *every* fact, static while the record turns. The summary carries three and delegates the rest, because a full fact list stacked beneath a full-bleed record on a phone is the "form" failure this section already rejected for the back face, arriving on a different surface. The card's height is therefore constant regardless of how much the record records, which is what lets the record's size above it be chosen rather than guessed.

**Both shapes send the reader to the same place for the rest.** The stacked summary's tap and the flanking panel both reach `/records/:id`, which is also where §10b's keyboard-reachable list already links every record — one destination, three routes to it, so they cannot describe different facts.

**The facts live in fixed panels beside the record, where there is room for them.** Artist, title, year, label, catalogue number, pressing details, condition, and purchase information — laid out beside the object, static while it turns, as the reference does. They do not track the record's geometry and never need to agree with it about anything. Below the A32 threshold they move to the stacked summary above; everything else in this section — the faces, the turn, the tilt, the hinge, reduced motion — is the same at every width.
