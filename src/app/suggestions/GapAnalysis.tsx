...
export function GapAnalysis({
  ...
  if (!configured) {
     ...
     */
    return (
      <p className="rounded-md border border-border bg-muted/30 p-3 text-prose text-muted-foreground">
    ...
    <div className="rounded-md border border-border p-4">
      <div className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-prose text-muted-foreground">
        ...
        */}
        {genres.length > 0 && (
          <select
            ...
            className="h-9 shrink-0 rounded-xs border border-border bg-background px-2 text-label"
          >
        ...
        <button
          ...
          className="shrink-0 rounded-xs border border-border px-3 py-1.5 text-label transition-colors hover:bg-muted disabled:opacity-60"
        >
      ...
      {state.phase === 'error' && (
        <p role="status" className="mt-3 text-prose text-destructive">
          {state.message}
        </p>
      )}
      ...
      {state.phase === 'done' && (
        <div className="mt-4">
          ...
          */}
          {asked !== null && (
            <p data-testid="asked-line" className="mb-1 text-meta text-muted-foreground">
              {askedLine({
          ...
          {state.suggestions.length === 0 ? (
            <p className="text-prose">No gaps suggested for this collection.</p>
          ) : (
            <ul className="space-y-3">
              {state.suggestions.map((suggestion) => (
                ...
                >
                    ...
                    <h3
                      className={
                        suggestion.onWantList === true
                          ? 'text-detail font-medium text-muted-foreground line-through'
                  ...
                  <p className="mt-1 text-prose text-muted-foreground">{suggestion.reason}</p>
                  <p className="mt-1 text-meta text-muted-foreground">{suggestion.genre}</p>
                </li>
              ))}
            </ul>
          )}
          ...
          */}
          {prior !== null && (
            <details data-testid="previous-analysis" className="mt-4 border-t border-border pt-3">
              ...
              */}
              <p data-testid="previous-asked-line" className="mt-2 text-meta text-muted-foreground">
                {askedLine({
